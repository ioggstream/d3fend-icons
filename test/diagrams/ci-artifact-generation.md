...
subgraph gh["GitHub"]
  subgraph &example-repo ["example-repo"]
    ...
    action -->|d3f:executes| precommit@{shape: process, label: "d3f:StaticAnalysisTool pre-commit hooks", icon: "d3f:FileFormatVerification"} -->|d3f:analyzes| code
    action -->|d3f:executes| codeql@{shape: process, label: "d3f:StaticAnalysisTool CodeQL", icon: "d3f:StaticAnalysisTool"} -->|d3f:analyzes| code

    action -->|d3f:executes| zizmor@{shape: process, label: "d3f:SystemVulnerabilityAssessment zizmor", icon: "d3f:DefensiveTechnique"} -->|d3f:analyzes| action
    zizmor -.->|d3f:enforces| least-priv["d3f:AccessControlConfiguration least-privilege permissions"]@{shape: doc}
    zizmor -.->|d3f:enforces| sha-pin["d3f:FileHashing sha256 artifact pinning"]@{shape: doc, icon: "d3f:DigitalArtifact"}

    action -->|d3f:executes| build@{shape: process, label: "d3f:BuildTool Build", icon: "d3f:Process"} -->|d3f:reads| code
  ...
  subgraph example-repo-artifact["example-repo-artifact"]
