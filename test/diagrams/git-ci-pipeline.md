...
subgraph local
    ...
    dev-mfa-token[d3f:Credential d3f:Multi-factorAuthentication dev]@{icon: "d3f:MultiFactorAuthentication"}
...
subgraph gh["GitHub"]
  ...
  subgraph gh-user-acl["d3f:AccessControlConfiguration GitHub User ACL"]
    ...
    gh-user-mfa[d3f:AccessControlConfiguration GitHub User MFA]@{shape: doc, icon: "d3f:MultiFactorAuthentication"}
  end
...
subgraph &example-repo ["example-repo"]
  ...
  %-codeowners["d3f:AccessControlConfiguration GitHub Code Owners"]@{shape: doc, icon: "d3f:AccessControlConfiguration"}
  %-action["d3f:Process GitHub Action"]@{icon: "d3f:Process"}
  %-prereceive["d3f:Process d3f:StaticAnalysisTool pre-receive hook"]@{icon: "d3f:StaticAnalysisTool"}
  ...
  %-prereceive -->|d3f:executes| %-policy@{shape: process, label: "d3f:CodeAnalyzer Policy Check", icon: "d3f:CodeAnalyzer"} -->|d3f:analyzes| push
